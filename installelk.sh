...
sed -ie 's/#server.host: "localhost"/server.host: "0.0.0.0"/g' /etc/metricbeat/metricbeat.yml
sed -ie 's/#server.name: "your-hostname"/server.name: '`hostname`'/g' /etc/metricbeat/metricbeat.yml
systemcl enable --now metricbeat
sudo metricbeat modules enable system
sudo metricbeat setup -e
sudo service metricbeat start

yum -y install filebeat
sed -ie 's/#server.host: "localhost"/server.host: "0.0.0.0"/g' /etc/filebeat/filebeat.yml
sed -ie 's/#server.name: "your-hostname"/server.name: '`hostname`'/g' /etc/filebeat/filebeat.yml
systemcl enable --now filebeat
